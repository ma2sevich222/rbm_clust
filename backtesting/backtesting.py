...
class Trade:
    ...
    def pl(self):
        ...
        commission = self.__broker._commission if self.__broker._commission_type == "absolute" else 0
        # Change by AT
        # before self.__size * (price - self.__entry_price)
        return self.__size * (price - self.__entry_price) * (self.__broker._features_coeff * 10 or 1) - commission
...
class _Broker:
    def __init__(self, *, data, cash, commission_type, commission, margin,
                 trade_on_close, hedging, features_coeff, exclusive_orders, index):
        assert 0 < cash, f"cash should be >0, is {cash}"
        if commission_type == "percent":
            assert -.1 <= commission < .1, \
                ("percent commission should be between -10% "
                 f"(e.g. market-maker's rebates) and 10% (fees), is {commission}")
        assert 0 < margin <= 1, f"margin should be between 0 and 1, is {margin}"
        self._data: _Data = data
        self._cash = cash
        self._commission_type = commission_type
        self._features_coeff = features_coeff
    ...
    def _adjusted_price(self, size=None, price=None) -> float:
        ...
        """
        if self._commission_type == "percent":
            adjusted_price = (price or self.last_price) * (1 + copysign(self._commission, size))
        else:
            adjusted_price = (price or self.last_price)
...
class Backtest:
    ...
    def __init__(self,
        ...
        if not isinstance(commission_type, str):
            raise TypeError('`commission_type` must be a str value (percent or absolute)')
        if commission_type != "percent" and commission_type != "absolute":
            raise TypeError('`commission_type` must be a str value (percent or absolute)')
        if not isinstance(commission, Number):
            raise TypeError('`commission` must be a float value')
        ...
        self._broker = partial(
            _Broker, cash=cash, commission_type=commission_type, commission=commission,
            margin=margin, trade_on_close=trade_on_close, hedging=hedging, features_coeff=features_coeff,
