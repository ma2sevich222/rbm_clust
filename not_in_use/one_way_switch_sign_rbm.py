...
out_data_root = f"oneway_switched_rbm_{source_file_name[:-4]}_{date_xprmnt}"
...
end_test_index = df[df["Datetime"] == end_forward_time].index[0] + 1
...
print(train_df)
forward_df = df_for_split[train_window:train_window + forward_window + patch -1]
