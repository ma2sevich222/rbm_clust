...
out_data_root = f"trades_zero_switched_rbm_{source_file_name[:-4]}_{date_xprmnt}"
...
def objective(trial):
    ...
    train_backtest_window = trial.suggest_categorical("train_backtest_window", [1760, 3520, 5280, 7040, 8800])
    forward_window = trial.suggest_categorical(
        "forward_window", [176, 440, 880, 1320,  1760]
    )
    ...
    for n in range(n_iters):
        ...
        if switch_signals == 1:
            Signals.loc[Signals["Signal"] == 0, "Signal"] = -111
            Signals.loc[Signals["Signal"] == 1, "Signal"] = -1
            Signals.loc[Signals["Signal"] == -111, "Signal"] = 1
        else:
    ...
    '''if switch_signals == 1:
        signals_combained.loc[signals_combained["Signal"] == 0, "Signal"] = -111
        signals_combained.loc[signals_combained["Signal"] == 1, "Signal"] = -1
        signals_combained.loc[signals_combained["Signal"] == -111, "Signal"] = 1
    else:
