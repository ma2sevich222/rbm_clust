...
from utilits.functions import get_train_test, get_stat_after_forward,dbscan_predict
...
out_data_root = f"switched_epsion_dbs_rbm_{source_file_name[:-4]}_{date_xprmnt}"
...
def objective(trial):
    ...
    for n in range(n_iters):
        ...
        db_cl = DBSCAN(eps=eps, min_samples=n_neighbors).fit(feature_set)
        '''df_pca["Label"] = features_labels
        fig = px.scatter(df_pca, x="param_1", y="param_2", color="Label")
        fig.show()'''
        uniq_labeles = np.unique(db_cl.labels_)
        print(f'Количество кластеров : {len(uniq_labeles)}')
        ...
        for forward_array in Forward_X:
            ...
            pred = dbscan_predict(db_cl ,trnsfrmd_forward_array.reshape(1, -1).astype(float))
    ...
    signals_combained.loc[signals_combained["Signal"] == -1, "Signal"] = -111
    signals_combained.loc[signals_combained["Signal"] == 1, "Signal"] = 112
    signals_combained.loc[signals_combained["Signal"] == 0, "Signal"] = 1
    signals_combained.loc[signals_combained["Signal"] == -111, "Signal"] = 0
    signals_combained.loc[signals_combained["Signal"] == 112, "Signal"] = -1
